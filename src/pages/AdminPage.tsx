...
function AdminPage() {
    ...
    const cancelMember = useCallback(async (memberId: string) => {
        setIsProcessing(true);
        try {
            await service.cancelMember(memberId);
        } catch (err) {
            log(PAGE_SOURCE, 'Не удалось отменить обслуживание.', err);
            toast.error(`Не удалось отменить обслуживание. ${errorMessage(err, '')}`.trim());
        } finally {
            setIsProcessing(false);
        }
    }, []);
