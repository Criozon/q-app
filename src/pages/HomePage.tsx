...
import type { ConfirmationState, SavedQueue, ServiceDraft } from '../types/domain';
...
function HomePage() {
  ...
  const [services, setServices] = useState<ServiceDraft[]>([{ id: uuidv4(), name: '', window_indices: [] }]);
  const [isLoading, setIsLoading] = useState(false);
  const [myQueues, setMyQueues] = useMyQueues();
  const navigate = useNavigate();
  
  const lastServiceInputRef = useRef<HTMLInputElement>(null);
  ...
  const handleUpdateService = (updatedService: ServiceDraft) => {
  ...
  const handleDeleteQueue = (queueToDelete: SavedQueue) => {
      setConfirmation({
          isOpen: true,
  ...
  const handleKeyPress = (event: KeyboardEvent<HTMLInputElement>) => {
    if (event.key === 'Enter' && !isLoading) void handleCreateQueue();
  };
